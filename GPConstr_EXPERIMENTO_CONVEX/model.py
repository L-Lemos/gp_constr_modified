...
class GPmodel():
    ...
    def __init__(self, kernel, likelihood = 1, mean = 0, constr_likelihood = 1E-6, verbatim = True):
        ...
        self.constr_deriv_2 = None # List with partial second derivative constraints (NO CROSS DERIVATIVES ALLOWED)
    ...
    def __str__(self):
        ...
        if self.constr_bounded is None and self.constr_deriv is None and self.constr_deriv_2 is None:
            txt += '   No constraints \n'
        else:
            ls = []
            ...
            if self.constr_deriv_2 is not None:
                ls = ls + ['d^2f/dx_' + str(i+1+len(self.constr_deriv)) + ' [{}]^2'.format(len_none(self.constr_deriv_2[i].Xv)) for i in range(len(self.constr_deriv_2))]
    ...
    def optimize(self, include_constraint = True, fix_likelihood = False, bound_min = 1e-6, conditional = False, pc_alg = 'minimax_tilting', n = 100, opt_method = 'L-BFGS-B', sigma_bounds = None):
        """
        ...
        if has_constr and include_constraint:
            ...
            self._optimize_constrained(fix_likelihood = fix_likelihood, bound_min = bound_min, conditional = conditional, algorithm = pc_alg, n = n, opt_method = opt_method, sigma_bounds = sigma_bounds)
            ...
            self._optimize_unconstrained(method = 'ML', fix_likelihood = fix_likelihood, bound_min = bound_min, opt_method = opt_method, sigma_bounds = sigma_bounds)
    ...
    def find_XV_subop(self, p_target, Omega = None, batch_size = None, bounds = None, i_range = None, nu = None, max_iterations = 200, moment_approximation = False, num_samples = 1000, min_prob_unconstr_xv = None, sampling_alg = 'minimax_tilting', moment_alg = 'correlation-free', opt_method = 'differential_evolution', print_intermediate = True):
        ...
        if i_range is None:
            ...
            if self.constr_deriv_2 is not None:
                i_range = i_range + [i+1+len(self.constr_deriv) for i in range(len(self.constr_deriv_2))]
        ...
        for i in i_range:
            if i == 0:
                ...
                F = 'd^2f/dx_{}^2'.format(i - (len(self.constr_deriv)))
            label.append(F)
        ...
        for j in range(max_iterations):
            ...
            if pc_min >= p_target:
                ...
                elif (i_min > 0) & (i_min <= len(self.constr_deriv)):
                    self.constr_deriv[i_min-1].add_XV(x_min)
                elif (i_min > len(self.constr_deriv)):
                    self.constr_deriv_2[i_min-1-len(self.constr_deriv)].add_XV(x_min)
    ...
    def _optimize_unconstrained(self, method = 'ML', fix_likelihood = False, bound_min = 1e-6, opt_method = 'L-BFGS-B', sigma_bounds=None):
        """
        ...
        # Run optimization
        if method == 'ML':
            res = self._optimize_unconstrained_ML(fix_likelihood, bound_min, opt_method, sigma_bounds=sigma_bounds)
        elif method == 'CV':
            print('TODO...')
    ...
    def _optimize_constrained(self, fix_likelihood = False, opt_method = 'shgo', bound_min = 1e-6, conditional = False, algorithm = 'minimax_tilting', n = 10E3, opt_args = {}, bounds = None, sigma_bounds=None):
        """
        Optimize hyperparameters of unconstrained GP
        ...
        if bounds is None:
            ...
            if fix_likelihood:
                ...
                if sigma_bounds is None:
                    bounds = [(1e-6, likelihood_scale*theta[0])]
                else:
                    bounds = sigma_bounds
                    
                bounds = bounds + [(bound_min, ker_var_scale*theta[1])]
    ...
    def _argmin_pc_subop(self, i, nu, bounds, opt_method = 'differential_evolution', moment_approximation = False, sampling_alg = 'minimax_tilting', moment_alg = 'correlation-free', verbatim = False, num_samples = 1000, return_res = False):
        ...
        if i == 0:
            ...
            label = 'a < df/dx_{} < b'.format(i)
        elif i >= (1 + len(self.constr_deriv)):
            label = 'a < d^2f/dx_{}^2 < b'.format(i - (len(self.constr_deriv)))
    ...
    def _constrprob_xs_2_momentapprox(self, XS, i, nu, algorithm = 'minimax_tilting', verbatim = False):
        ...
        C~(XS) | Y, C
        """
        assert 1 == 0, 'Como colocar derivada segunda neste def inteiro (_constrprob_xs_2_momentapprox)?'
        ...
        assert 1 == 0, 'Como colocar derivada segunda nas linhas abaixo?'
        # Prior mean
        if i == 0:
            # Boundedness
            Lmu = np.matrix(self.mean*np.ones(len(XS))).T
    ...
    def _check_constraints(self):
        ...
        if self.constr_deriv_2 is not None:
            i = 1 + len(self.constr_deriv)
            for c in self.constr_deriv_2:
                c._check(self.kernel.dim, '2nd Derivative ' + str(i))
                i+=1
           
    def _optimize_unconstrained_ML(self, fix_likelihood = False, bound_min = 1e-6, opt_method = 'L-BFGS-B', sigma_bounds=None):
        """
        ...
        if fix_likelihood: 
            ...
            bounds = [(bound_min, None)]*num_params 
            if sigma_bounds is None:
                sigma_bounds = [(1e-6, None)]
            bounds = sigma_bounds + bounds
    ...
    def __has_xv_deriv_2(self):
        """ Check if there are virtual points for second derivative constraints """
        if self.constr_deriv_2 is None:
            return False
        else:
            for constr in self.constr_deriv_2:
                if constr.Xv is not None:
    ...
    def __has_xv(self):
        ...
        return self.__has_xv_bounded() or self.__has_xv_deriv() or self.__has_xv_deriv_2()
