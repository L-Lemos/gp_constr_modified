...
class constrained_gp_model():
    ...
    def __init__(self,input_dim,unconstrained,first_deriv,second_deriv):
        ...
        if not(self.unconstrained):
            ...
            model.constr_likelihood = 1E-6 #1E-6
    ...
    def train_model(self, x_train, y_train, virtual_points, sigma_bounds=[(1E-6,1E-4)]):
        ...
        if self.unconstrained:
            ...
            # try:
            #     model.optimize(include_constraint = True, fix_likelihood = False, bound_min = 0.5, sigma_bounds=sigma_bounds)                          
            # except: # If constrained optimization fails, do unconstrained optimization
            #     model.optimize(include_constraint = False, fix_likelihood = False, bound_min = 0.5, sigma_bounds=sigma_bounds)
    ...
    def predict(self,x_test):                  
        ...
        if self.unconstrained:
            mean, var = model.calc_posterior_unconstrained(x_test, full_cov=False) #, compute_mode = False, num_samples = 10000, save_samples = 30, algorithm = 'minimax_tilting', resample = False)
        else:
